...
class Agent:
    ...
    def choose_action(self, observation):
        ...
        value = self.critic(state)
        action = action.cpu().detach().tolist()[0]

        # prob = T.squeeze(dist.log_prob(action)).item()
        prob = np.log(np.mean(action))
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                ...
                print(f"{new_probs = }")
                print(f"{new_probs.mean(axis=1).exp() = }")
                print(f"{advantage = }")
                print(f"{old_probs = }")
                prob_ratio = new_probs.mean(axis=1).exp() / old_probs.mean(axis=1).exp()
